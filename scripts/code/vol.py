...
def get_sink_data() -> tuple[int, bool, int]:
    p = subprocess.Popen(
    ...
    id_, mute, vol = re.search(
        r'ID: sink-(\d+).*?Mute: (\d).*?(\d+)%.*?Default',
        str(p.stdout.read(), encoding="utf8")
    ).groups()

    return id_, mute == "1", int(vol)

if __name__ == "__main__":
    if len(argv) == 1:
        _, mute, vol = get_sink_data()
        print(f"{vol: 3} " + ("𝄽 " if mute else "𝅘𝅥𝅮 "))
    else:
        id_, _, _ = get_sink_data()
        what_do = argv[1]
        if what_do in {"+", "-"}:
